    ...
    <div className="bg-white border border-slate-200 rounded-[2rem] p-6 md:p-10 shadow-sm">
      ...
      <div className="grid grid-cols-7 gap-px bg-slate-100 border border-slate-100 rounded-3xl overflow-hidden shadow-inner">
        ...
        {cells.map((c, i) => {
          ...
          const isAdminView = isAdmin && (isSunday || isSaturday) && c.status !== "past";
            ...
            >
              ...
              {!isAdmin && c.label && (
                <div className={cn(
                  ...
                  c.status === "mine" ? tagColors.monthly : c.status === "fixed" ? tagColors.fixed : c.status === "pending" ? tagColors.pending : "bg-slate-100 text-slate-500 border-slate-200"
                )}>
                  {c.label}
                </div>
              )}
              {!isAdmin && c.status === "available" && !isBlocked && (
