...
export function DocumentImportForm() {
  ...
  // 🆕 Estado para marcar se o documento é um adiantamento
  ...
  const handleAprovarTudo = async () => {
    ...
    try {
      ...
      for (const result of pageResults) {
        ...
        // 🆕 Inserção com a flag is_adiantamento
        const { error: insertError } = await supabase.from("documentos").insert({
          ...
          is_adiantamento: isAdiantamento, // 🔥 novo campo
        });
      ...
      setTimeout(() => {
        ...
        setCurrentPage(0);
        setIsAdiantamento(false); // reseta o checkbox após salvar
      }, 2000);
    } catch (err) {
  ...
  if (pageResults.length === 0) {
    return (
      <div className="space-y-4">
        ...
        {/* 🆕 Checkbox para adiantamento */}
        <div className="flex items-center gap-2">
          <input
            type="checkbox"
            id="adiantamento"
            checked={isAdiantamento}
            onChange={(e) => setIsAdiantamento(e.target.checked)}
            className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
          />
          <Label htmlFor="adiantamento" className="text-sm cursor-pointer">
            Este documento é um adiantamento?
          </Label>
        </div>
