...
export async function ensureDocumentosSchema() {
  if (!schemaPromise) {
    schemaPromise = supabase.rpc("exec_sql", {
      sql: `
        ...
        CREATE POLICY "documentos_update_policy" ON public.documentos        FOR UPDATE TO authenticated USING (auth.uid() = colaborador_id);
